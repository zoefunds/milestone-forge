...
**Known gap**: none of these tables have a `contract_address` column. `grant_id`/`milestone_id`/`challenge_id` are sequential counters assigned by the contract *starting fresh on every new deployment*, not globally unique — so after redeploying to a new `MILESTONE_FORGE_CONTRACT_ADDRESS`, cached rows from the previous deployment can collide by primary key with the new deployment's own IDs (e.g. both deployments independently produce a `grant-3`) and the indexer will serve stale data under the new grant's own ID with no error. Until the schema adds a `contract_address` column, truncate `grants`/`milestones`/`challenges`/`contract_events` manually after every redeploy to a new address (see `docs/DEPLOYMENT.md`). This caused a real, confusing bug during the fourth deploy's live testing — see `memory/MEMORY.md`.
